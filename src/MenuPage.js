          ...
          <div key={item.id} className={"qr-item" + (!item.isAvailable? ' unavailable':'')} onClick={() => handleOpen(item)}>
            <div className="qr-img-wrap">
              <img
                ...
                alt={item.name}
                onError={(e)=>{ if(e.currentTarget.dataset.fallback) return; e.currentTarget.dataset.fallback='1'; e.currentTarget.src=PLACEHOLDER_IMG; }}
              />
      ...
      {modalItem && (
        <div className="modal-overlay" onClick={()=>setModalItem(null)}>
          <div className="menu-item-modal" onClick={e=>e.stopPropagation()}>
            ...
            {/* Item Image */}
            <div className="item-image-container" style={{ '--modal-image-url': `url(${modalItem.image || PLACEHOLDER_IMG})` }}>
              <img 
                src={modalItem.image || PLACEHOLDER_IMG}
                alt={modalItem.name}
            ...
            <div className="item-content">
              ...
              <div className="customizations-section">
                <h3 className="customizations-title">Customizations:</h3>
                
                <div className="customization-options">
                  {modalItem.customizations && modalItem.customizations.length > 0 ? (
                    modalItem.customizations.map((c,i)=>(
                      <label key={i} className="customization-option">
                        <input
                          type="checkbox"
                          checked={selectedCustomizations[c] || false}
                          onChange={() => handleCustomizationChange(c)}
                          className="customization-checkbox"
                        />
                        <span className="customization-text">{c}</span>
                      </label>
                    ))
                  ) : (
                ...
                <button 
                  className="confirm-customization-btn"
                  onClick={()=>{ 
                    const itemWithCustomizations = {
                      ...modalItem,
                      customizations: selectedCustomizations,
                      customizationId: Date.now() // unique ID for this customized version
                    };
                    updateQty(modalItem.id, 1); 
                    onSelectAdd && onSelectAdd(itemWithCustomizations); 
                    setModalItem(null); 
                  }}
                >
